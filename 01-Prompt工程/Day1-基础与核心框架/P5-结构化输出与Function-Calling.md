# P5：结构化输出（JSON Mode）与 Function Calling 基础

## 目标
让大模型输出规范的 JSON 格式数据，掌握 Function Calling 的基本用法

## 为什么要结构化输出？

| 场景 | 非结构化输出的问题 | 结构化输出的好处 |
|------|------------------|-----------------|
| 提取数据 | 格式不一致，难以解析 | 直接 `json.loads()` |
| 调用外部 API | 无法提取参数 | 自动映射到函数参数 |
| 构建 Agent | 无法判断意图 | 明确函数调用指令 |

### 常用结构化格式对比

除了 JSON，Prompt 中还可以要求其他格式：
...
### 进阶用法：用 JSON Mode 做推理（Inference Engine）
...
## 方法三：Function Calling（工具调用）

通过 `tools` 参数注册函数描述，让模型自主判断何时调用哪个函数，并提取参数。这是 Agent 的基础能力。
...
当前使用 `deepseek-v4-flash`（思考模型），当进行工具调用时需注意：
...
- [ ] 能用 Prompt 引导模型输出 JSON
- [ ] 能使用 JSON Mode 强制格式化输出
- [ ] 理解 Function Calling 的原理和流程
- [ ] 跑通了带 tool_choice 的函数调用示例
- [ ] 实现了"调用函数→返回结果→模型回答"的完整循环

## 下一步
完成 → 进入 [P6-Prompt模板化](P6-Prompt模板化.md)
